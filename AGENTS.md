...
- Game modes: ENDLESS (default survival) or MISSION, toggled by a HUD chip (persisted as `zoombie-mode`, applied via `Game.setMode()`). In mission mode the HUD shows `wave/target` and completing `MISSION_TARGET_WAVE` (see `missionComplete` in `logic.ts`) triggers `win()`: victory fanfare, confetti, and a "YOU WIN!" overlay instead of endless scaling.
